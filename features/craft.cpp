...
static void __fastcall HookCraftEntry(void* __this) {
...
void InitCraft() {
    ...
    for (int i = 0; Signatures::CraftPartner[i] && !g_CraftPartnerAddr; i++) {
        ...
        if (a) {
            g_CraftPartnerAddr = a;
            Log("[SCAN] CraftPartner: matched pattern[%d] at 0x%llX | sig: %s", i, (unsigned long long)a, Signatures::CraftPartner[i]);
        }
    }
    if (!g_CraftPartnerAddr) Log("[SCAN] CraftPartner: FAILED - no pattern matched");

    uintptr_t addr = ScanLog(Signatures::CraftEntry, "CraftEntry");
