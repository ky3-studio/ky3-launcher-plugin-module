...
namespace PatternScanner {
    ...
    inline uintptr_t ScanLog(const char* patterns[], const char* name) {
        for (int i = 0; patterns[i] != nullptr; i++) {
            uintptr_t addr = ScanMainModule(patterns[i]);
            if (addr) {
                Log("[SCAN] %s: matched pattern[%d] at 0x%llX | sig: %s", name, i, (unsigned long long)addr, patterns[i]);
                return addr;
            }
        }
        Log("[SCAN] %s: FAILED - no pattern matched", name);
        return 0;
    }

    inline uintptr_t ScanLogValidated(const char* patterns[], const char* name) {
        for (int i = 0; patterns[i] != nullptr; i++) {
            uintptr_t addr = ScanMainModule(patterns[i]);
            if (addr && IsValidFunctionPrologue(addr)) {
                Log("[SCAN] %s: matched pattern[%d] at 0x%llX (validated) | sig: %s", name, i, (unsigned long long)addr, patterns[i]);
                return addr;
            } else if (addr) {
                Log("[SCAN] %s: pattern[%d] hit 0x%llX but FAILED prologue validation", name, i, (unsigned long long)addr);
            }
        }
        Log("[SCAN] %s: FAILED - no pattern matched", name);
        return 0;
    ...
    inline uintptr_t ResolveCallLog(uintptr_t addr, const char* name) {
        uintptr_t resolved = ResolveRelativeAddress(addr, 1, 5);
        if (resolved) {
            Log("[RESOLVE] %s: 0x%llX -> 0x%llX", name, (unsigned long long)addr, (unsigned long long)resolved);
        } else {
            Log("[RESOLVE] %s: FAILED from 0x%llX", name, (unsigned long long)addr);
        }
        return resolved;
    }
}
